...
# ALL CUSTOMERS IN DB
# 1. Creare la connessione al database
# 2. Creare un oggetto di tipo cursore, tramite il metodo .cursor()
# 3. Preparazione della query SQL (INSERT INTO per aggiungere, DELETE FROM per rimuovere,
#       UPDATE per modificare un determinato campo, SELECT per selezionare dei dati)
# 4. Esecuzione della query tramite metodo .execute()
# *** DB CONNECTION ***
...
def remove_customer():
    ...
    if name_removed and surname_removed:
        ...
        cursor.execute(query, valori)
        db.commit()
        # label of successful remove
        label_rem = tkinter.Label(window, text = "Customer removed", fg = "red")
        label_rem.grid(row = 3, column = 2, columnspan = 2)
    else:
        tkinter.messagebox.showwarning("Error in delete customer", "Compile all fields")
